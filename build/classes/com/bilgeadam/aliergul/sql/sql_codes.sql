...
AS
	$$
		...
		BEGIN
			...
			last_user:= (SELECT log_user_id FROM login_log_history WHERE log_id=last_log);
		
			wrong_login_attempts_total:= (SELECT COUNT(*) FROM (SELECT * FROM login_log_history WHERE log_user_id = last_user ORDER BY log_login_date DESC LIMIT 5) as a WHERE a.log_is_successful=false);
